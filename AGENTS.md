...
You are using Baron through the Codex adapter.
...
- At session start, silently run `baron capability check --adapter codex`; capability presence is not execution evidence.
- At session start, silently run `baron runtime check --adapter codex`; unsafe, missing, or unverified required backends cannot satisfy proof.
- At session start, silently run `baron context --codex` and follow its bounded context.
...
- When prior conversation context may matter, rely on `baron context --codex --task "<task>"` or `baron session-replay search "<query>"`; do not dump full session histories.
